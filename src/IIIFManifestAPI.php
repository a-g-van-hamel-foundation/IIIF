...
class IIIFManifestAPI extends ApiBase {
	...
	public function execute() {
		...
		$manifestArr = IIIFJson::getArrayFromManifest( $manifest );
		if ( $manifestArr == null ) {
			return;
		}
		$host = IIIFUtils::getUrlBase();
		$currentId = $host . "$_SERVER[REQUEST_URI]";
		// Get version number of IIIF Presentation API
		$presVersion = self::getPresentationApiVersion( $manifestArr );

		$obj = ( array_key_exists( 'obj', $params) ) ? $params['obj'] : false;
		// print_r( "<br>" . $urlObject . "<br>" );
		$smwQueryRaw = ( array_key_exists( 'smwquery', $params ) ) ? $params['smwquery'] : false;
		//$smwQueryProcessed = ( $smwQueryRaw == null ) ? "" : IIIFSMW::processSemanticQuery( $smwQueryRaw, $obj );
		$smwConfig = ( array_key_exists( 'smwconfig', $params ) ) ? $params['smwconfig'] : false;
		$annotPageId = ( array_key_exists( 'annotid', $params ) ) ? $params['annotid'] : false;
